  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent hideCloseOnMobile className="sm:max-w-4xl p-0 gap-0 overflow-hidden flex flex-col dark:bg-[#1e2a3a]">
        ...
        {/* Header */}
        <div className="px-4 sm:px-8 pt-2 sm:pt-6 pb-2 sm:pb-5 border-b border-slate-200/80 dark:border-slate-700/50 flex-shrink-0">
          <div className="flex items-start gap-2 sm:gap-3">
            <motion.button onClick={handleStatusToggle} whileTap={{ scale: 0.85 }} className={cn("w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 mt-0.5", isCompleted ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-300 hover:border-emerald-400")}>
        ...
        <div className="flex-1 overflow-hidden flex flex-col sm:flex-row min-h-0">
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden order-2 sm:order-1">
            <div className="flex-1 overflow-y-auto no-scrollbar">
              ...
              {/* Description / Notes */}
              <div className="px-4 sm:px-8 pt-2 sm:pt-6 pb-2 sm:pb-5">
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <span className="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-slate-400">Description</span>
              ...
              <div className="px-4 sm:px-8 pb-4">
                <div className="border-t border-slate-200/80 dark:border-slate-700/50 pt-3 sm:pt-5">
                  <span className="text-[11px] sm:text-xs font-semibold uppercase tracking-wider text-slate-400">Activity</span>
                  <div className="mt-3 sm:mt-4 space-y-4 sm:space-y-5">
                    ...
                    {comments.length === 0 && (<div className="text-center py-4 sm:py-8"><MessageSquare className="w-6 h-6 sm:w-8 sm:h-8 text-slate-200 mx-auto mb-1 sm:mb-2" /><p className="text-xs sm:text-sm text-slate-400">No comments yet</p></div>)}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="px-3 sm:px-6 py-2 sm:py-4 border-t border-slate-200/80 dark:border-slate-700/50 bg-slate-50/60 dark:bg-[#151d2b] flex-shrink-0">
              <div className="flex gap-2 sm:gap-3">
                ...
                <div className="flex-1 relative">
                  <Textarea ref={textareaRef} value={comment} onChange={handleCommentChange} onPaste={handlePaste} onFocus={() => { setTimeout(() => textareaRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 300); }} placeholder="Write a comment... @ to mention" className="min-h-[44px] sm:min-h-[52px] resize-none pr-10 bg-white dark:bg-[#1a2535] dark:border-slate-600 rounded-xl text-sm focus:ring-[#0069AF]/20 focus:border-[#0069AF]/40" />
                  {isNativeCamera ? (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
          ...
          <div className="order-1 sm:order-2 sm:w-[280px] border-b sm:border-b-0 sm:border-l border-slate-200/80 dark:border-slate-700/50 bg-slate-50/40 dark:bg-[#162032] overflow-y-auto no-scrollbar flex-shrink-0">
            ...
            <div className="px-3 py-2 sm:p-5">
              ...
              <div className="sm:mt-4 grid grid-cols-2 gap-x-2 gap-y-0 sm:grid-cols-1 sm:block sm:space-y-1">
                {/* Status */}
                <div className="flex items-center py-1 sm:py-2 px-2 rounded-lg hover:bg-slate-100/80 dark:hover:bg-slate-700/40 transition-colors sm:-mx-2 relative">
                  ...
                  <AnimatePresence>{showStatusDropdown && (<><div className="fixed inset-0 z-[99]" onClick={() => setShowStatusDropdown(false)} /><motion.div initial={{ opacity: 0, y: -4, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -4, scale: 0.95 }} transition={{ duration: 0.15 }} className="absolute top-full left-16 mt-1 bg-white dark:bg-[#1e2a3a] border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-[100] overflow-hidden min-w-[160px]">
                ...
                {/* Due Date */}
                <div className="flex items-center py-1 sm:py-2 px-2 rounded-lg hover:bg-slate-100/80 dark:hover:bg-slate-700/40 transition-colors sm:-mx-2">
                  <span className="text-[11px] sm:text-xs text-slate-500 w-16 sm:w-20 flex-shrink-0">Due date</span>
                ...
                {/* Priority */}
                <div className="flex items-center py-1 sm:py-2 px-2 rounded-lg hover:bg-slate-100/80 dark:hover:bg-slate-700/40 transition-colors sm:-mx-2">
                  <span className="text-[11px] sm:text-xs text-slate-500 w-16 sm:w-20 flex-shrink-0">Priority</span>
            ...
            {/* Checklist */}
            <div className="border-t border-slate-200/80 dark:border-slate-700/50 px-3 py-2 sm:p-5">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div className="flex items-center gap-2">
                          ...
                          <div key={item.id}>
                            ...
                            <Draggable draggableId={item.id} index={index}>
                              {(dragProvided, dragSnapshot) => (
                                <div ref={dragProvided.innerRef} {...dragProvided.draggableProps} className={cn("rounded-lg transition-all", dragSnapshot.isDragging && "shadow-lg ring-2 ring-blue-200 dark:ring-blue-800 bg-white dark:bg-[#1e2a3a] z-50")}>
                                  <div className={cn("flex items-center gap-1.5 group py-1.5 px-1.5 rounded-lg hover:bg-slate-100/80 dark:hover:bg-slate-700/40 transition-colors cursor-pointer", item.completed && "opacity-50")} onClick={() => setExpandedChecklistItem(isExpanded ? null : item.id)}>
