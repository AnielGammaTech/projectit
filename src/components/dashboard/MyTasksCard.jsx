...
export default function MyTasksCard({ tasks = [], parts = [], projects = [], currentUserEmail, onTaskComplete, inline = false }) {
  ...
  // ─── Inline mode: compact horizontal strip ───
  if (inline) {
    const topItems = myTasks.slice(0, 5);
    if (totalItems === 0) return null;

    return (
      <div className="bg-white dark:bg-[#1e2a3a] rounded-xl border border-slate-200/80 dark:border-slate-700/50 px-4 py-2.5">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 shrink-0">
            <div className="p-1.5 rounded-lg bg-[#0069AF]/10 dark:bg-blue-900/30">
            ...
            <div>
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-500 ml-1.5">{totalItems} assigned</span>
            </div>
          </div>
          ...
          <div className="flex-1 flex items-center gap-2 overflow-x-auto scrollbar-none min-w-0">
            {topItems.map((task) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs whitespace-nowrap shrink-0 transition-colors border",
                    dueStatus === 'overdue'
                      ...
                      : dueStatus === 'soon'
                        ...
                        : "bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700/50 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50"
                  )}
                >
            ...
            {totalItems > 5 && (
              <span className="text-[10px] text-slate-400 dark:text-slate-500 shrink-0 px-1">+{totalItems - 5} more</span>
            )}
          </div>
          <Link to={createPageUrl('AllTasks')} className="shrink-0">
            <Badge variant="outline" className="hover:bg-slate-100 dark:hover:bg-slate-700 text-[10px] h-6">View All</Badge>
          </Link>
        </div>
      </div>
    );
