  ...
  const content = (
    <motion.div
      ...
      className={cn(
        ...
        (href || onClick) && "hover:shadow-md hover:border-slate-200 cursor-pointer"
      )}
    >
      <div className="flex items-start justify-between">
        <div>
          <p className={cn("text-sm leading-tight", highlight ? "text-orange-700 font-medium" : "text-slate-500")}>{title}</p>
          <p className={cn("text-2xl font-bold mt-2", highlight ? "text-orange-600" : "text-slate-900")}>{value}</p>
          {subtitle && (
            <p className={cn("text-xs mt-0.5", highlight ? "text-orange-500" : "text-slate-400")}>{subtitle}</p>
          )}
        </div>
        <div className={cn(
          "p-2 rounded-lg",
