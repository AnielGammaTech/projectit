    ...
    <div className="min-h-screen bg-background text-foreground p-3 sm:p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">{format(currentTime, 'EEEE, MMMM d, yyyy • h:mm a')}</p>
            </div>
          </div>
        </div>
        <Button onClick={generateInsights} disabled={loadingInsights} variant="outline" className="text-foreground border-border">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-muted/50 rounded-xl p-3 text-center">
                <p className="text-xl sm:text-3xl font-bold text-foreground">{activeProjects.length}</p>
        ...
        <div className="col-span-2">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card rounded-2xl p-5 h-full">
            <div className="flex items-center gap-3 mb-4">
            ...
            <div className="space-y-3">
              {techWorkload.map((tech, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{tech.hoursToday}h logged today</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {tech.overdueCount > 0 && (
                  ...
                  <Progress value={Math.min(tech.taskCount * 10, 100)} className="h-1.5 bg-muted" />
                </motion.div>
              ))}
        ...
        {/* Right - AI Insights */}
        <div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-card rounded-2xl p-5 h-full">
            <div className="flex items-center gap-3 mb-4">
            ...
            {loadingInsights ? (
              ...
              <p className="text-muted-foreground text-center py-8">Click refresh to generate insights</p>
            )}
          </motion.div>
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-6 bg-card rounded-2xl p-5">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 rounded-lg bg-red-500/20">
          ...
          {overdueTasks.slice(0, 8).map(task => {
            ...
            return (
              <div key={task.id} className="bg-muted/50 rounded-xl p-3 border border-red-500/20">
                ...
                <p className="text-xs text-muted-foreground truncate">{task.assigned_name || 'Unassigned'}</p>
