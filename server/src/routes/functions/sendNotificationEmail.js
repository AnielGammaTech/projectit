...
export default async function handler(req, res) {
  try {
    const { to, type, title, message, projectId, projectName, fromUserName, link } = req.body;

    if (!to || !title) {
      return res.status(400).json({ success: false, error: 'Missing required fields: to, title' });
    }

    // Get user notification preferences
    let prefs = {};
    try {
      const userSettings = await entityService.filter('NotificationSettings', { user_email: to });
    ...
    if (prefKey && prefs[prefKey] === false) {
      ...
      return res.json({ success: true, skipped: true, reason: 'User disabled this notification type' });
    }
    ...
    let appConfig = {};
    try {
      const appSettings = await entityService.filter('AppSettings', { setting_key: 'main' });
