...
Deno.serve(async (req) => {
  try {
    ...
    const url = `${baseUrl}/api/functions/updateQuoteStatus`;

    console.log(`Updating quote status at: ${url}`);
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ quote_id, status })
    });
    ...
    if (!response.ok) {
      ...
      return Response.json({ 
        success: false, 
        error: `Failed to update QuoteIT status (${response.status}): ${text}` 
      });
    }
